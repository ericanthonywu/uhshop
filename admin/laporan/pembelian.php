<script>
if (0 != $("#laporan_pembelian_date").length) {
	var e = $("#laporan_pembelian_date"), t = moment(), a = moment();
	e.daterangepicker({
...
$(document).ready(function () {
    $('.ranges ul li').click(function () {
        ...
        $.ajax({
            type: "POST",
            cache: false,
            url: "<?=$base_url?>ajax/pembelian.php",
            data: {
                data: data
            },
            success: function (result) {
                $('#laporan_pembelian').DataTable().ajax.url("<?=$base_url?>table/laporan_pembelian.php?data=" + result).load();
            }
        });
    });
